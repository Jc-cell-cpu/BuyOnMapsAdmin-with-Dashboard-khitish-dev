...
// Define the data structure for dynamic stats
interface StatItem {
  icon: JSX.Element;
  color: string;
  title: string;
...
const DataStatsOne: React.FC<dataStats> = async () => {
  ...
  try {
    totalUsers = await fetchAllUsers(); // Returns number (e.g., 26)
    const postData = await fetchAllPosts(); // Returns object (e.g., { "total Posts": 24, ... })
    totalPosts = postData['total Posts'] || 0; // Extract total Posts, fallback to 0
    totalAppointments = await fetchAllAppointments(); // Returns number (e.g., 28)
  } catch (error) {
    console.error('Error fetching stats:', error);
    // Optionally set fallback values or render an error state
  }

  // Dynamic stats from APIs
  const dataStatsList: StatItem[] = [
    {
      icon: (
        <svg
          width="26"
      ...
      color: '#3FD97F',
      title: 'Total Views',
      value: '--', // Static, as no API provided
      growthRate: '--', // Indicate no data
    },
    {
      icon: (
        <svg
          ...
          >
            ...
            <div className="mt-6 flex items-end justify-between">
              ...
              {item.growthRate === '--' ? (
                ...
                <span
                  className={`flex items-center gap-1.5 text-body-sm font-medium ${typeof item.growthRate === 'number' && item.growthRate > 0 ? 'text-green' : 'text-red'
                    }`}
                >
                  {item.growthRate}%
